# Boundaries: When to Use bd vs session checklist

This reference provides detailed decision criteria for choosing between bd issue tracking and session checklist for task management.

For Codex runtime, "session checklist" means a session-local execution checklist in prompt/notes.
...
- [Decision Matrix](#decision-matrix)
  ...
  - [Use session checklist for](#use-session-checklist-for): Single-Session Tasks, Linear Execution, Immediate Context, Simple Tracking
- [Detailed Comparison](#detailed-comparison)
- [Integration Patterns](#integration-patterns)
  - Pattern 1: bd as Strategic, session checklist as Tactical
  - Pattern 2: session checklist as Working Copy of bd
...
- [Common Mistakes](#common-mistakes)
  - Using session checklist for multi-session work, using bd for simple tasks, not transitioning when complexity emerges, creating too many bd issues, never using bd
...
- If markdown skim would suffice → **session checklist is fine**

This heuristic captures the essential difference: bd provides structured context that persists across long gaps, while session checklist excels at immediate session tracking.
...
**Why session checklist wins**: Simple checklist is perfect for linear execution. No need for persistence or dependencies. Clear completion within session.
...
**Why session checklist wins**: Steps are predetermined and sequential. No discovery, no blockers, no side quests. Just execute top to bottom.
...
**Why session checklist wins**: No external context to track. Everything needed is in current conversation. session checklist provides user visibility, nothing more needed.
...
**Why session checklist wins**: User wants to see thinking and progress. session checklist is visible in conversation. bd is invisible background structure.

---

## Detailed Comparison

| Aspect | bd | session checklist |
|--------|-----|-----------|
...
bd and session checklist can coexist effectively in a session. Use both strategically.

### Pattern 1: bd as Strategic, session checklist as Tactical

**Setup:**
- bd tracks high-level issues and dependencies
- session checklist tracks current session's execution steps
...
session checklist (for JWT validation):
- [ ] Install JWT library
...
- Create session checklist checklist from bd issue's acceptance criteria
- Update bd as session checklist items complete

**Example:**
```
Session start:
- Check bd: "issue-auth-42: Add JWT token validation" is ready
- Extract acceptance criteria into session checklist
- Mark bd issue as in_progress
- Work through session checklist items
- Update bd design notes as you learn
- When session checklist completes, close bd issue
```
...
**How to transition:**
```
1. Create bd issue with current session checklist content
2. Note: "Discovered this is multi-session work during implementation"
3. Add dependencies as discovered
4. Keep session checklist for current session
5. Update bd issue before session ends
6. Next session: resume from bd, create new session checklist if needed
```

**From bd to session checklist:**
...
2. Create session checklist from issue description
3. Execute via session checklist
...
**session checklist role**: None initially. Might use session checklist for single-session testing sprints once migration scripts ready.

### Example 2: Simple Feature Implementation

**Scenario**: Add logging to existing endpoint based on clear specification.

**Why session checklist**:
- Single session work
...
**Initial assessment**: Seems simple, try session checklist first.
...
**session checklist role**: Could use session checklist for individual controller updates as implementing.
...
### Mistake 1: Using session checklist for Multi-Session Work
...
**Solution**: Use session checklist. It's designed for exactly this case.

### Mistake 3: Not Transitioning When Complexity Emerges

**What happens**:
- Start with session checklist for "simple" task
- Discover blockers and dependencies mid-way
- Keep using session checklist despite poor fit
...
### Mistake 5: Never Using bd Because session checklist is Familiar
...
**"This looks straightforward"** → session checklist

**As work progresses:**

✅ **Stays straightforward** → Continue with session checklist, complete in session
...
**Time horizon:**
- Same session → session checklist
- Multiple sessions → bd

**Dependency structure:**
- Linear steps → session checklist
- Blockers/prerequisites → bd

**Scope clarity:**
- Well-defined → session checklist
- Exploratory → bd

**Context complexity:**
- Conversation has everything → session checklist
- External context needed → bd

**User interaction:**
- User watching progress → session checklist visible in chat
- Background work → bd invisible structure

**Resume difficulty:**
- Easy from markdown → session checklist
