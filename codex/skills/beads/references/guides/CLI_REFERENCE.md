...
**See also:**
- [TROUBLESHOOTING.md - Sandboxed environments](TROUBLESHOOTING.md#sandboxed-environments-codex-codex-code-etc) for detailed sandbox troubleshooting
...
**When to use:**
- Use `allow` (default) for daily imports and routine pull/push workflows
...
### Dolt Remote Operations (v0.56+)

```bash
# Pull remote Dolt commits into local database
bd dolt pull

# Push local Dolt commits to remote
bd dolt push
```

`bd sync` is deprecated in v0.56+ and kept as a no-op compatibility command.
...
bd dolt push  # Push local Dolt commits to remote when configured
```

**Use `bd dolt push` at session end** when a Dolt remote is configured.
