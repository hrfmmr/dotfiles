...
Terminology note: "session checklist" means a local execution checklist used during a single session.
...
- [Integration with session checklist](#integration-with-session-checklist) - Using both tools together
...
- [ ] Reconstruct session checklist list from notes if needed
```
...
### At Session Start (Codex's responsibility)
...
### At Session End (Codex prompts user)
...
```
User Tips:
- [ ] When stopping work: Let Codex suggest notes update
- [ ] When resuming: Let Codex read notes and report context
...
# Codex reports: "Markdown→Docs feature is 80% parsed. We were testing
...
## Integration with session checklist
...
- [ ] Create session checklist from acceptance criteria for execution
- [ ] Work through session checklist items
- [ ] Update bd design notes as you learn
- [ ] When session checklist complete, close bd issue
```

**Why hybrid**: bd provides persistent structure, session checklist provides visible progress.
...
**Should I create a bd issue or use session checklist?**
...
1. Transition from session checklist to bd mid-session
