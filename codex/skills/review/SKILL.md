...
- Only the orchestrator may emit the final `decision/findings/summary` artifact or post the consolidated PR comment.
...
5. Emit one final artifact; post the consolidated comment only in `pr-comment` mode.
...
- If unresolved lane failure or aggregation failure remains, do not post a PR comment; return control to the caller with an explicit failure reason.
